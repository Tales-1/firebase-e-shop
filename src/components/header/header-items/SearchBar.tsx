    ...
    return(
            <form className={`${searchBarStyles} relative flex items-stretch md:m-auto md:w-1/2 xl:w-1/3`}>   
                ...
                <div className="relative w-full">
                    ...
                    <input type="text" onChange={(e)=>setQuery(e.target.value)} value={query} id="simple-search" className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-xl focus:ring-blue-500 focus:border-blue-500 block w-full pl-10 p-2.5  dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500 py-3" placeholder="Search" required />
                </div>
                <button type="submit" 
                ...
                <div className={`fixed flex flex-col left-0 top-0 bottom-0 bg-white ${dropdownStyles} w-screen overflow-scroll pt-10 duration-[500ms] transition-all origin-top -z-10`}>
                    ...
                    <ul className="mt-24 border-t-2 border-black w-1/2 mx-auto flex flex-col">
                        {filtered.map((item,i)=>{
                            return( 
