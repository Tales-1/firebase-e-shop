...
const ProductCard:React.FC<Props> = ({name,price,urls,id}) => { 
    ...
    return(
        <motion.article 
            initial={{opacity:0,}}
            animate={{opacity:1,transition:{duration:.5, delay:0.6, ease:"easeInOut"}}}
            whileHover={{scale:1.1}}
            className="flex flex-col items-center 
                        min-h-card w-full max-w-[12rem] md:max-w-[14rem] lg-2:max-w-[17rem] rounded-lg will-change-transform">
                
            <div className="grid relative shadow-lg h-full w-full">
                <Link to={`${pathname}/${name?.split(" ").join("-")}`} className="row-start-1 col-start-1" 
            ...
            <h3 className="text-lg md:text-xl font-bold font-serif text-center">{name}</h3>
